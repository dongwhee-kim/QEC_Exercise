...
- **Specific Distillation Protocols [1]**
    ...
    - **7-to-1 Distillation** (Appendix D in **[1]**)
        - Summry: A protocol for distilling $|Y\rangle$ magic states, based on the **Steane Code**.
    ...
    - **Triorthogonal Codes & 20-to-4** (Fig. 16 in [1])
        - Summry: An expanded family of protocols based on large mathematical matrices (Triorthogonal matrices) that guarantee **Transversal T-gates**.
...
- **Surface-code Implementation (Fig. 17 in [1])**
    - Summry: The hardware implementation strategy that maps magic state distillation circuits onto 2D Surface Code Tiles and discrete **Time steps ($1\circlearrowright$)**.
...
- **Benchmarking (Table 1 in [1])**
    - Summry: Proves the extreme **space-time cost reduction** of the Tile-based lattice surgery strategy.
...
- **Higher-fidelity Protocols & Concatenation (Fig. 19 in [1])**
    - Summry: Multi-level distillation architectures designed to achieve **extremely low error rates ($\sim 10^{-21}$)**.
